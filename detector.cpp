	...
	std::vector<int>& classId,int i)
{
	...
	blob->getTensorDesc().reshape(v, NCDHW);*/
	//cout <<"dims:" << blob->getTensorDesc().getDims()[0] << endl;
	//cout << v[0] << endl;

	const int net_grid_h = static_cast<int>(blob->getTensorDesc().getDims()[2]);    // 80/40/20   H
	const int net_grid_w = static_cast<int>(blob->getTensorDesc().getDims()[3]);    // 80/40/20   W
	const int batch_size = static_cast<int>(blob->getTensorDesc().getDims()[0]);    // batch_size  N
	int n = blob->getTensorDesc().getDims().size();//5(应该是NCDHW)
	cout << "blob 's type:" << typeid(blob).name() << endl;
	//const int anchor_num = 3;
	const int anchor_num = static_cast<int>(blob->getTensorDesc().getDims()[1]);    // anchor_num,3   C
	// item_size为类别个数，对于COCO而言，输出为80
	const int item_size = static_cast<int>(blob->getTensorDesc().getDims()[4]);     // (4+1+class_num) D
	//const int item_size = 85;//(一个种类是6)
	...
	//cout<<"尺寸" << (sizeof(output_blob)) << endl;
	cout <<_msize(output_blob) << endl;
	cout << sizeof(float*) << endl;
	//cout << output_blob[2000000] << endl;
	...
	for (int n = 0; n < anchor_num; ++n)
		for (int i = 0; i < net_grid; ++i)
			for (int j = 0; j < net_grid; ++j) {
				...
				//double obj_prob = sigmoid(output_blob[n * ggi + i * gi + j * item_size + 4]);//max_cof
				// 获取输出目标框
...
bool Detector::init(std::string xml_path) {
	try {
		...
		_input_name = inputInfo.begin()->first;//输入头的名称,私有属性
		...
		cout<<input->getInputData()->getLayout()<<endl;
		   /**C - for 1 - dimensional,
		...
		for (auto& output : _outputinfo) {
			...
			cout << typeid(output.second).name() << endl;//class std::shared_ptr<class InferenceEngine::Data>
			output.second->setPrecision(InferenceEngine::Precision::FP32);
		}
		//获取可执行网络
		_network = ie.LoadNetwork(cnnNetwork, "CPU");//时间长
		return true;
	}
...
bool Detector::process_frame(Mat& curFrame, std::vector<Object>& detectedObj)
{
	...
	float* blob_data = blobMapped.as<float*>();//as<T>()类型转换为T
	...
	for (auto& output : _outputinfo) {
		...
		InferenceEngine::TensorDesc t = blob->getTensorDesc();
		cout << "推理得到的Layout：" << t.getLayout() << endl;
		//blob->getTensorDesc().setLayout(ANY);
		cout <<"Layout:" << blob->getTensorDesc().getLayout() << endl;
	...
	for (int i = 0; i < blobs.size(); i++) {
		...
		//解析blobs
		parseYolov5(blobs[i], th, origin_rect_temp, origin_rect_cof_temp, classId,i);
